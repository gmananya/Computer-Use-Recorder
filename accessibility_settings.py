...
# Reads screen-reader (NVDA, JAWS) and Windows accessibility toggle settings,
# and provides a diff utility used by the recorder to track a11y state changes.
...
def _to_bool(v):
    """Convert registry/INI string values like '1', 'true', 'yes', 'on' to bool (or None if unrecognised)."""
...
def _ci_get(d: dict, key: str, default=None):
    """Case-insensitive dict lookup; returns default if the key is absent."""
...
def _ci_section(cfg: dict, sec_name: str):
    """Return the dict for a named INI section using case-insensitive matching, or {} if absent."""
...
def _parse_with_configparser(text: str):
    """Parse INI text with ConfigParser (case-preserving, no interpolation)."""
...
def _nvda_user_dir():
    """Locate the NVDA user configuration directory (installed or portable)."""
    # Installed path
...
def _jaws_user_settings_dir():
    """Return the JAWS per-user settings/enu directory for the most recent installed version, or None."""
    appdata = os.environ.get("APPDATA") or ""
...
def _jaws_shared_settings_dir():
    """Return the shared (ProgramData) JAWS settings/enu directory for the newest version, or None."""
...
def get_accessibility_state():
    """Return a snapshot of active Windows accessibility settings (Narrator, magnifier, contrast, etc.)."""
...
def diff_accessibility(prev, curr):
    """Return a dict of changed keys between two get_accessibility_state() snapshots, or None if no changes."""
